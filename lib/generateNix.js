...
const { resolvedToName, urlToName } = require('./urlToName')
...
function fetchgit(fileName, url, rev, branch, builtinFetchGit, fullFileName) {
  return `    {
    name = "${fullFileName}";
    path =
      let${builtinFetchGit ? `
...
function fetchLockedDep(builtinFetchGit) {
  return function (pkg) {
    ...
    const fullFileName = resolvedToName(resolved)
    ...
    if (url.startsWith('git+') || url.startsWith("https://github.com")) {
      ...
      const urlForGit = url.replace(/^git\+/, '') + (url.startsWith("https://github.com") ? ".git" : "")

      return fetchgit(fileName, urlForGit, rev, branch || 'master', builtinFetchGit, fullFileName)
    }
